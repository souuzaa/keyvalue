...
export async function createServer(options: ServerOptions = {}) {
  const dataDir = options.dataDir ?? process.env.KV_DATA_DIR ?? path.join(process.cwd(), "data");
  ...
  const server = Bun.serve({
    port: options.port ?? (process.env.PORT ? Number(process.env.PORT) : 3000),
    fetch(req, server) {
...
if (import.meta.main) {
  ...
  const quiet = process.env.KV_QUIET === "1";
  if (!quiet) {
